	...
	public static void main(String[] args) throws Exception {
		...
		for(int n = 0; n < docs.length(); n++)
		{
		   object = (JSONObject) docs.getJSONObject(n).get("request");
		    ...
		    JSONArray jsArr =  (JSONArray) object.get("queryString");
		    ...
		    if(object.get("url").toString().indexOf("https://www.google-analytics.com/") != -1){
				...
				for(int n1 =0; n1 < jsArr.length() ; n1++){
					object1.put("QSName",  jsArr.getJSONObject(n1).get("name"));
					object1.put("QSVal",  jsArr.getJSONObject(n1).get("value"));
					System.out.println("qsObj name::"+  jsArr.getJSONObject(n1).get("name"));
		...
		try{
	      	  File file=new File("/opal/fromJSON1.csv");
